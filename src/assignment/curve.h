...
struct CurvePoint
{
	...
	FW::Mat4f getTransformation()
	{
		FW::Mat4f matrix;
		matrix.setCol(0, FW::Vec4f(N, 0.0f));
		matrix.setCol(1, FW::Vec4f(B, 0.0f));
		matrix.setCol(2, FW::Vec4f(T, 0.0f));
		matrix.setCol(3, FW::Vec4f(V, 1.0f));
		return matrix;
	}
